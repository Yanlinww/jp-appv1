...
function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        else if (view === 'exam_year_select') {
           setInitialSwipePage(1); // ✨ 設定回到 Page 1
           setView('level_select');
        }
        else if (view === 'exam') {
          if (window.confirm('確定退出考試？')) setView('exam_year_select');
        }
        else if (view === 'quiz') {
  ...
  // ✨ 新增：當回到 level_select 時，根據 initialSwipePage 滑動
  useEffect(() => {
    if (view === 'level_select' && swipeRef.current) {
      const width = swipeRef.current.clientWidth;
      swipeRef.current.scrollTo({ left: width * initialSwipePage, behavior: 'auto' }); // auto 比較快，不會有視覺延遲
    }
  ...
  const openExamLevel = (lvl: string) => {
    setExamLevel(lvl);
    setView('exam_year_select');
  };

  // ✨ 退出年份選擇頁的處理函式
  const exitExamYearSelect = () => {
    setInitialSwipePage(1); // 告訴系統要滑到第 2 頁 (考古題頁)
    setView('level_select');
  };
  ...
  const openExam = (paper: ExamPaper) => {
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // 只在使用者主動滑動時更新 activePage，避免干擾程式控制的 scrollTo
        ...
        <div className="swipe-wrapper" ref={swipeRef} onScroll={handleScroll} onMouseDown={onMouseDown} onMouseLeave={onMouseLeave} onMouseUp={onMouseUp} onMouseMove={onMouseMove}>
          ...
          <div className="swipe-page">
            <div className="home-screen">
              <div className="hero-section">
                <div className="hero-icon">📜</div>
                <div className="app-title">考古題特訓</div>
                <div className="app-subtitle">JLPT 歷屆試題</div>
              </div>
              ...
              <div className="menu-grid">
                ...
                {['N1', 'N2', 'N3', 'N4', 'N5'].map((lvl) => (
                  <button 
                    key={lvl} 
                    className="btn menu-card exam-card" 
                    onClick={() => openExamLevel(lvl)}
                  >
                    <div className={`level-badge exam-badge ${lvl.toLowerCase()}`}>{lvl}</div>
                    <div className="level-info">歷屆試題庫</div>
                  </button>
                ))}
              </div>
            </div>
  ...
  // 考古題年份選擇頁
  if (view === 'exam_year_select') {
    return (
      <div className="app-container">
        <div className="home-screen">
          <div className="hero-section">
            <div className="current-level-tag">{examLevel}</div>
            <div className="app-subtitle">請選擇年份</div>
          </div>

          <div className="menu-grid">
            <button onClick={exitExamYearSelect} className="btn-ghost" style={{marginBottom: 20}}>← 返回上一頁</button>

            {examLevel === 'N5' ? (
              <>
                <div className="exam-year-title">2010-2011</div>
                <button className="btn menu-card exam-card" onClick={() => openExam(exam2010_written)}>
                  <div className="level-badge exam-badge n5">文</div>
                  <div className="level-info">文字・語彙・文法・読解</div>
                </button>
                <button className="btn menu-card exam-card" onClick={() => openExam(exam2010_listening)}>
                  <div className="level-badge exam-badge n5" style={{background:'#20c997'}}>聴</div>
                  <div className="level-info">聴解 (聽力)</div>
                </button>
              </>
            ) : (
              <div style={{textAlign: 'center', padding: 40, color: '#868e96'}}>
                🚧 {examLevel} 試題整理中...<br/>敬請期待！
              </div>
            )}
          </div>
        </div>
  ...
  if (view === 'exam' && selectedExam) {
    return (
      <ExamView 
        paper={selectedExam} 
        onExit={() => setView('exam_year_select')} // ✨ 退出後回到年份選擇
      />
    );
  }

  // ... (其餘頁面保持不變，為了節省篇幅省略) ...
  // 請保留 Basic Numbers, Short Phrases, Home, Grammar, Quiz, Result 的代碼
  ...
  if (view === 'result') { return (<div className="app-container"><div className="result-screen"><div className="score-section"><div className="score-circle"><div className="score-number">{score}</div><div className="score-label">分 (共{quizHistory.length}題)</div></div><h2 style={{marginBottom: 20}}>測驗結束！🎉</h2></div><div className="review-list"><h3 style={{marginLeft: 10, color: '#868e96'}}>答案解析</h3>{quizHistory.map((log, i) => (<div key={i} className={`review-item ${log.isCorrect ? 'correct' : 'wrong'}`}><div className="review-q"><span className="q-num">{i + 1}.</span> {log.question.w} {log.question.w !== log.question.r && <span style={{fontSize:'0.8rem', color:'#adb5bd', marginLeft:8}}>({log.question.r})</span>}</div><div className="review-detail">{log.isCorrect ? (<span className="ans-tag correct">答對</span>) : (<span className="ans-tag wrong">答錯</span>)}<div className="ans-text">{!log.isCorrect && (<div className="user-ans">你選: {log.userAnswer}</div>)}<div className="correct-ans">正解: {log.correctAnswer}</div></div></div></div>))}</div><div className="result-footer"><button onClick={() => setView('quiz_setup')} className="btn btn-primary" style={{marginBottom: 12}}>再測一次</button><button onClick={() => setView('home')} className="btn btn-outline">回儀表板</button></div></div></div>); }
