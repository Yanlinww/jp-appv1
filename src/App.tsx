...
import { allLevels, n5GrammarList, shortPhrasesList, particleQuestionsData, type Word, type Grammar } from './data';
...
function App() {
  ...
  const APP_VERSION = "Ver 2025.12.26";
      ...
      const saveData = async () => {
        try {
          await setDoc(doc(db, "users", user.uid), {
            savedWords,
            settings: { notifyTime, streakReminder, weekendDND, username, avatar, darkMode },
  ...
  }, [savedWords, notifyTime, streakReminder, weekendDND, username, avatar, darkMode, user, isCloudSyncing]);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        else if (view === 'particle_quiz') setView('level_select'); // ✨ 助詞測驗返回
        else if (view === 'quiz_setup') setView('home');
        else if (view === 'exam_year_select') { setInitialSwipePage(2); setView('level_select'); } // 改為回到第 3 頁
  ...
  useEffect(() => {
    if (view === 'level_select' && swipeRef.current) {
      ...
      setActivePage(initialSwipePage);
    }
  ...
  const scrollToPage = (pageIndex: number) => {
    if (swipeRef.current) {
      const width = swipeRef.current.clientWidth;
      swipeRef.current.scrollTo({ left: width * pageIndex, behavior: 'smooth' });
  ...
  const exitExamYearSelect = () => { setInitialSwipePage(2); setView('level_select'); }; // ✨ 退出考古題回到第 3 頁
...
const handleLogout = () => {
    if (window.confirm("確定要登出嗎？")) {
      logout();
      setAvatar(null); // ✨ 清除目前頭像
      localStorage.removeItem('jp_user_avatar'); // ✨ 清除 localStorage
    }
  };
...
<div className="mini-avatar" onClick={goToSettings}>
  <img src={avatar || DEFAULT_AVATAR} alt="User" />
</div>
          </div>
        )}
        ...
        <div className="swipe-wrapper" ref={swipeRef} onScroll={handleScroll} onMouseDown={onMouseDown} onMouseLeave={onMouseLeave} onMouseUp={onMouseUp} onMouseMove={onMouseMove}>
          ...
          <div className="swipe-page">
            <div className="home-screen">
              ...
              <div className="menu-grid" style={{paddingBottom: 0}}>
                ...
                <button 
                  onClick={() => alert('N2 資料庫建置中，敬請期待！')} 
                  className="btn menu-card level-card n2"
                  style={{opacity: 0.8}} // 稍微透明一點表示未開放
                >
                  <div className="level-badge">N2</div>
                  <div className="level-info">商業應用 (準備中)</div>
                </button>

                <button 
                  onClick={() => alert('N1 資料庫建置中，敬請期待！')} 
                  className="btn menu-card level-card n1"
                  style={{opacity: 0.8}}
                >
                  <div className="level-badge">N1</div>
                  <div className="level-info">專業高階 (準備中)</div>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="swipe-page">
            <div className="home-screen">
              <div className="hero-section" style={{marginBottom: 20}}>
                ...
                <div className="app-title">學習工具</div>
                <div className="app-subtitle">加強基礎與應用</div>
              </div>
              <div className="tools-container">
                <div className="tool-card" onClick={() => setView('basic_numbers')}>
                  <div className="tool-icon-wrapper" style={{background:'#e3fafc', color:'#15aabf'}}>#</div>
                  <div className="tool-info"><div className="tool-title">基本數詞</div><div className="tool-desc">時間、日期、量詞</div></div>
                  <div className="tool-arrow">›</div>
                </div>
                <div className="tool-card" onClick={() => setView('short_phrases')}>
                  <div className="tool-icon-wrapper" style={{background:'#fff0f6', color:'#e64980'}}>🗣️</div>
                  <div className="tool-info"><div className="tool-title">實用短句</div><div className="tool-desc">日常會話、問候</div></div>
                  <div className="tool-arrow">›</div>
                </div>
                <div className="tool-card" onClick={() => {
                  // ✨ 啟動助詞測驗 (使用 QuizView，但傳入不同的題庫)
                  setView('particle_quiz');
                }}>
                  <div className="tool-icon-wrapper" style={{background:'#f3f0ff', color:'#7950f2'}}>🧩</div>
                  <div className="tool-info"><div className="tool-title">助詞特訓</div><div className="tool-desc">は、が、に、を、で</div></div>
                  <div className="tool-arrow">›</div>
                </div>
              </div>
            </div>
          ...
          {/* Page 3: Exams (Moved from Page 2) */}
          <div className="swipe-page">
...
<div className="avatar-circle" onClick={handleAvatarClick}>
  {/* 如果有 avatar 就用 avatar，沒有就用 DEFAULT_AVATAR */}
  <img src={avatar || DEFAULT_AVATAR} alt="Avatar" className="avatar-image" />
</div>                <div className="profile-info"><div className="profile-name" onClick={() => { const name = prompt('請輸入新暱稱', username); if (name) setUsername(name); }}>{username} ✎</div><div className="profile-level">N5 學習中</div></div>
              </div>
              <div className="settings-content">
                <div className="group-title">帳號綁定</div>
                <div className="settings-group">
                  {user ? (
                    <div className="settings-item" onClick={handleLogout}><div className="item-left"><div className="item-icon" style={{background:'#ff3b30'}}>🚪</div>登出 ({user.displayName || user.email})</div><div className="item-right">›</div></div>
                  ) : (
                    <div className="settings-item" onClick={loginWithGoogle}><div className="item-left"><div className="item-icon" style={{background:'#34c759'}}>G</div>使用 Google 登入</div><div className="item-right">同步資料 ›</div></div>
                  )}
                </div>
        ...
        <div className="bottom-nav-bar">
          <button className={`nav-item ${activePage === 0 ? 'active' : ''}`} onClick={() => scrollToPage(0)}>
            <div className="nav-icon">🗻</div>
            <span className="nav-label">首頁</span>
          </button>
          <button className={`nav-item ${activePage === 1 ? 'active' : ''}`} onClick={() => scrollToPage(1)}>
            <div className="nav-icon">🧰</div>
            <span className="nav-label">工具</span>
          </button>
          <button className={`nav-item ${activePage === 2 ? 'active' : ''}`} onClick={() => scrollToPage(2)}>
            <div className="nav-icon">✍️</div>
            <span className="nav-label">試題</span>
          </button>
          <button className={`nav-item ${activePage === 3 ? 'active' : ''}`} onClick={() => scrollToPage(3)}>
            <div className="nav-icon">⚙️</div>
  ...
  if (view === 'particle_quiz') {
    return (
      <QuizView 
        // 轉換格式：確保 explanation 被傳入
        list={particleQuestionsData.map(q => ({ 
          id: q.id, 
          q: q.q, 
          options: q.options, 
          a: q.a,
          explanation: q.explanation // ✨ 關鍵：把解析傳進去
        }))}
        count={10} // ✨ 設定隨機出題數：10 題
        onFinish={(finalScore, history) => { 
           // ✨ 這裡要修改：考完試要跳轉到 result 頁面看解析，而不是直接 alert
           setScore(finalScore);
           setQuizHistory(history);
           setView('result'); 
        }} 
        onExit={() => { setView('level_select'); setInitialSwipePage(1); }} 
      />
    );
  }
  ...
  if (view === 'quiz') { 
    return <QuizView list={getQuizSourceList()} count={quizCount} onFinish={handleQuizFinish} onExit={() => setView('home')} />; 
  }
if (view === 'result') { 
    return (
      <div className="app-container">
        <div className="result-screen">
          <div className="score-section">
            ...
            <h2 style={{marginBottom: 20, color: 'var(--text-main)'}}>測驗結果</h2>
          </div>
          
          <div className="review-list">
            <h3 style={{marginLeft: 10, color: 'var(--text-sub)', fontSize: '0.9rem', marginBottom: 10}}>詳細檢討</h3>
            
            {quizHistory.map((log, i) => {
              // 1. 先嘗試找單字詳細資料 (給一般單字測驗用)
              const wordDetails = allLevels.n5.find(w => w.w === log.question.q);
              
              // 2. 檢查題目本身有沒有附帶解析 (給助詞/考古題用)
              // @ts-ignore (忽略型別檢查，因為 ExamQuestion 擴充了 explanation 欄位)
              const questionExplanation = log.question.explanation;
                ...
                <div key={i} className={`review-item ${log.isCorrect ? 'correct' : 'wrong'}`}>
                  ...
                  <div className="review-header">
                    <div className="review-q">
                      <span style={{color:'var(--text-sub)', marginRight:8, fontSize:'0.9rem'}}>{i + 1}.</span> 
                      {log.question.q}
                    </div>
                    <span className={`status-text ${log.isCorrect ? 'correct' : 'wrong'}`}>
                      {log.isCorrect ? 'Correct' : 'Mistake'}
                    </span>
                  </div>
                  ...
                  <div className="review-body">
                    {!log.isCorrect && (
                      <div className="ans-row">
                        <span className="label-text">你選</span>
                        <span className="val-text wrong">{log.userAnswer}</span>
                      </div>
                    )}
                    <div className="ans-row">
                      <span className="label-text">正解</span>
                      <span className="val-text correct">{log.question.a}</span>
                    </div>
                  </div>

                  {/* ✨ 解析顯示區：優先顯示題目自帶解析，沒有的話才顯示單字意思 */}
                  {(questionExplanation || wordDetails) && (
                    <div className="review-expl">
                      ...
                      {questionExplanation && (
                        <div className="expl-row">
                          ...
                          <span className="expl-text">{questionExplanation}</span>
                        </div>
                      )}
                      ...
                      {!questionExplanation && wordDetails && (
                        <>
                          <div className="expl-row">
                            <span className="expl-tag">意思</span>
                            <span className="expl-text">{wordDetails.m}</span>
                          </div>
                          {wordDetails.p && (
                            <div className="expl-row">
                              <span className="expl-tag" style={{background:'rgba(255, 146, 43, 0.1)', color:'#ff922b'}}>詞性</span>
                              <span className="expl-text" style={{fontSize:'0.9rem', color:'var(--text-sub)'}}>{wordDetails.p}</span>
                            </div>
                          )}
                        </>
                      )}
          ...
          <div className="result-footer">
            <button onClick={() => {
              // 判斷是哪種測驗，決定「再測一次」要回哪裡
              // 簡單判斷：如果題目有 explanation 欄位，通常是助詞測驗
              const isParticleQuiz = quizHistory.length > 0 && (quizHistory[0].question as any).explanation;
              
              if (isParticleQuiz) {
                setView('particle_quiz'); // 重啟助詞測驗
              } else {
                setView('quiz_setup'); // 回到單字測驗設定
              }
            }} className="btn btn-primary" style={{marginBottom: 12}}>再測一次</button>
            <button onClick={() => { setView('level_select'); setInitialSwipePage(1); }} className="btn btn-outline">回工具頁</button>
          </div>
        </div>
      </div>
    ); 
