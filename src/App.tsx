...
import QuizView, { type QuizLog } from './QuizView'; // ✨ 引入 QuizLog 型別
import './App.css';
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        ...
        else if (view === 'result') setView('home'); // 結果頁按 ESC 回首頁
          ...
          <div className="word-list" ref={listRef}>
            {filteredList.length === 0 ? (<div style={{textAlign: 'center', padding: 40, color: '#868e96'}}>沒有找到資料 🍃</div>) : (
              filteredList.map((item) => (
                <div key={item.w} className="word-item" onClick={() => openDetail(item)} style={{cursor: 'pointer'}}>
    ...
    return (
      <div className="app-container">
        <div className="detail-screen">
          ...
          <div className="detail-footer"><button className="nav-btn" onClick={goToPrev} disabled={!hasPrev}>← 上一個</button><div className="nav-counter">{currentIndex + 1} / {filteredList.length}</div><button className="nav-btn" onClick={goToNext} disabled={!hasNext}>下一個 →</button></div>
        </div>
      </div>
    );
  }
      ...
      <div className="app-container">
        <div className="result-screen">
            ...
            {quizHistory.map((log, i) => (
              <div key={i} className={`review-item ${log.isCorrect ? 'correct' : 'wrong'}`}>
                ...
                <div className="review-detail">
                  {log.isCorrect ? (
                    <span className="ans-tag correct">答對</span>
                  ) : (
                    <span className="ans-tag wrong">答錯</span>
                  )}
                  <div className="ans-text">
                    {/* 如果答錯，顯示 你的答案 -> 正確答案 */}
                    {!log.isCorrect && (
                      <div className="user-ans">你選: {log.userAnswer}</div>
                    )}
          ...
          <div className="result-footer">
            <button onClick={() => setView('quiz')} className="btn btn-primary" style={{marginBottom: 12}}>
              再測一次
            </button>
